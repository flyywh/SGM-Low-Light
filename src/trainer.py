...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (lr, hr,  _, idx_scale) in enumerate(self.loader_train):
            ...
            hr_r_pre = self.model_R(lr_r.detach(), idx_scale)
            hr_r_pre = hr_r_pre

            hr_pre = hr_l_pre.detach()*hr_r_pre

            loss_I = - criterion_ssim(hr_l_pre, hr_l.detach())
            loss_F = - criterion_ssim(hr_pre, hr.detach())
            loss_R = - criterion_ssim(hr_r_pre, (hr/hr_l_pre).detach())

            ep_list_lr = [0.03, 0.06, 0.12, 0.18]
            ep_list_hr = [0.09, 0.18, 0.27, 0.36]
            ...
            map_loss = loss_I + loss_R + 100*loss_F

            sparse_coeff = 1e-5

            loss = rect_loss + sparse_coeff*(sparse_loss_lr + sparse_loss_hr) + map_loss*100 
                ...
                for idx_img, (lr, hr, filename, _) in enumerate(tqdm_test):
                    ...
                    if not no_eval:
                        eval_acc += utility.calc_psnr(
                            hr, hr_pre, scale, self.args.rgb_range,
